...
def _npz_from_checkpoint(checkpoint: Path) -> tuple[str, str, int]:
    import numpy as np
    import torch
    ...
    return base64.b64encode(raw).decode("ascii"), digest, len(raw)
...
def _runtime(encoded: str, max_moves: int) -> str:
    return f"""
...
def build(checkpoint: Path, max_moves: int) -> str:
    encoded, digest, size = _npz_from_checkpoint(checkpoint)
    ...
    parts.append(_runtime(encoded, max_moves))
    return "".join(parts)
